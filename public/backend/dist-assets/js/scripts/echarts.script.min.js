...
$(document).ready(function () {
  ...
  var months = JSON.parse(document.getElementById('monthname').value);
  var subs = JSON.parse(document.getElementById('subscription').value);
  var unsubs = JSON.parse(document.getElementById('unsubscription').value);
  var sub_max_round = Math.round(Math.max(...subs)/1000)*1000 + 1000;
  //var sub_max_round = Math.round(Math.max(subs)/1000);
  var months = months.map(element=>element.substring(0,3));
  ...
  if (echartElemBar) {
    ...
    echartBar.setOption({
      legend: {
        ...
        orient: 'horizontal',
        x: 'right',
        data: ['Unsubscription', 'Subscription']
      },
      grid: {
        left: '8px',
        right: '8px',
  ...
  var week_revenue = JSON.parse(document.getElementById('last_week_revenue').value);
  var week_revenue_r = week_revenue.map(element=>element/1000);
  var rev_max = Math.round(Math.max(...week_revenue_r));
  var week_sub = JSON.parse(document.getElementById('last_week_sub').value);
  var sub_min = Math.min(...week_sub);
  var week_dayname = JSON.parse(document.getElementById('last_week_dayname').value);
  var week_dayname = week_dayname.map(element=>element.substring(0,3));
