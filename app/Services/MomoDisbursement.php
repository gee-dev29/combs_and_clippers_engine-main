...
class MomoDisbursement extends Product
{
    /**
     * Product.
     *
    ...
    public function __construct(array $headers = [], array $middleware = [], ClientInterface $client = null)
    {
        ...
        $this->refundUri = $config->get('mtn-momo.products.disbursement.refund_uri');
        $this->refundStatusUri = $config->get('mtn-momo.products.disbursement.refund_status_uri');
    ...
    public function getRefundUri()
    {
        return $this->refundUri;
    }

    /**
     * @param string $transactionUri
     */
    public function setRefundUri($refundUri)
    {
        $this->refundUri = $refundUri;
    }

    public function getRefundStatusUri()
    {
        return $this->refundStatusUri;
    }

    /**
     * @param string $transactionStatusUri
     */
    public function setRefundStatusUri($refundStatusUri)
    {
        $this->refundStatusUri = $refundStatusUri;
    }
    ...
    public function refund($referenceIdToRefund, $transactionId, $amount, $payerMessage = '', $payeeNote = '')
    {
        $momoTransactionId = Uuid::uuid4()->toString();
        ...
        try {
            $this->client->request('POST', $this->refundUri, [
                'headers' => $headers,
                'json' => [
                    'amount' => $amount,
                    'currency' => $this->currency,
            ...
            throw new DisbursementRequestException('Request to refund transaction - unsuccessful.', 0, $ex);
        }
    }

    public function getRefundStatus($referenceId)
    {
        $refund_status_uri = str_replace('{referenceId}', $referenceId, $this->refundStatusUri);

        try {
            $response = $this->client->request('GET', $refund_status_uri, [
                'headers' => [
                    'X-Target-Environment' => $this->environment,
                ],
            ]);
            ...
            throw new DisbursementRequestException('Unable to get refund status.', 0, $ex);
        }
    }
}
